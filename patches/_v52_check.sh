#!/bin/bash
# v52 沙箱单元验收：在 NAS 上重建 /tmp/v52t 沙箱并跑 _v52_check.py
# 前置：app_v52.py 与 _v52_check.py 已上传到 /tmp/
set -u
SB=/tmp/v52t
...
# 复用真实 proxy 的兄弟模块（recommend.py 等），仅替换 app.py
for f in "$REAL"/*.py; do
  b=$(basename "$f")
  [ "$b" = "app.py" ] && continue
  cp -f "$f" "$SB/proxy/$b"
done
cp -f /tmp/app_v52.py "$SB/proxy/app.py"
echo "  proxy 文件: $(ls "$SB/proxy" | tr '\n' ' ')"

echo "=== 1) 造假 music.db（shared_library 表）==="
"$PY" - <<'PY'
import sqlite3
db = "/tmp/v52t/music.db"
...
cd "$SB" && "$PY" /tmp/_v52_check.py
rc=$?
echo
echo "=== 3) 探针内容（[scanreq]）==="
grep -a "\[scanreq\]" "$SB/home/access_probe.log" 2>/dev/null || echo "  （无）"
echo "=== 4) 退出码 $rc ==="
exit $rc
