...
- **新增的 18 个配置项全部有默认值**，`.env` 不填也能跑；想用「只对收藏落盘」和「自动扫库」按 `PATCHES.md` 里的清单打开即可。
- **回滚很简单**：换回上游的 `proxy/app.py` 并重启服务即可（本分支未改数据结构）。
- 本分支自带 4 份验收报告（[`reports/`](reports/)）与可复现脚本（[`patches/`](patches/)），
  单元验收共 **163 项**（`patches/_v54_check.py`）。
