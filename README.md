...
> ## ⚠️ 本仓库是**增强分支**
>
> 基于上游 [javycoder/fnos_music_ext](https://github.com/javycoder/fnos_music_ext) **v1.6.0**，
> 在其之上叠加了一批修复「手机 App 整列空白 / 播放 404 / 不自动扫库 / 孤儿歌词」的补丁。
>
> **改动只涉及一个文件：`proxy/app.py`**，其余文件与上游逐字节一致。
> 上游原有 **117 个函数、28 个路由一个都没删、没改名**（纯增量补丁）：
> 3,080 行 → 5,342 行，+82 个函数，+18 个配置项（全部有默认值）。
>
> ### 👉 想了解「和原版到底差在哪」，请看 **[DIFFERENCES.md](DIFFERENCES.md)**
> 逐项对比（上游行为 / 本分支行为 / 用户可见影响）、改动边界、哪些适合回馈上游，都在里面。
>
> 其他文档：完整改动说明与配置项见 **[PATCHES.md](PATCHES.md)**，
> 逐条验收报告见 [`reports/`](reports/)，可复现脚本见 [`patches/README.md`](patches/README.md)，
> 与原版的自动化结构对比见 [`patches/_diff_upstream.py`](patches/_diff_upstream.py)。
>
> 📌 验收报告与脚本中的账号名、云盘卷标识、曲库 guid 等已做**脱敏处理**，
> 以 `<user>` / `<vol-id>` / `<lib-guid>` 之类占位符表示；替换方式见
> [`patches/README.md`](patches/README.md) 的对照表。**生产环境只需要 `proxy/app.py` 一个文件**，
> `patches/` 与 `reports/` 只是改动的生成器与证据。
